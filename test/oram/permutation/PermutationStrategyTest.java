...
public class PermutationStrategyTest {
    ...
    public void shouldReturnTheSameBlocks() {
        assertNull(permutationStrategy.permuteEncryptedBlocks(null));

        List<BlockEncrypted> blocks = new ArrayList<>();
        assertThat("Empty list gives empty list", permutationStrategy.permuteEncryptedBlocks(blocks), is(blocks));

        BlockEncrypted block0 = new BlockEncrypted(Util.leIntToByteArray(1), Util.leIntToByteArray(2));
        blocks.add(block0);
        assertEquals("Array of 1 element must return the same", permutationStrategy.permuteEncryptedBlocks(blocks), blocks);
        ...
        blocks = permutationStrategy.permuteEncryptedBlocks(blocks);
