...
public class AccessStrategyLookaheadMult implements AccessStrategy {
    ...
    public AccessStrategyLookaheadMult(int size, int matrixHeight, byte[] key, Factory factory, int offset,
...
//        List<BlockLookahead> blocks = readBlocks(indexOfCurrentAddress, maintenanceColumnIndex, blockInColumn);
        BlockEncrypted blockEncrypted = communicationStrategy.read(position);
        if (blockEncrypted == null) {
            logger.error(prefix + "Block read from server were null");
            return null;
        }
        BlockLookahead block = decryptToLookaheadBlock(blockEncrypted);
        if (block == null) {
            logger.error(prefix + "Unable to decrypt block from server");
            return null;
        }
...
//        Write block back to the matrix
        ...
        if (!communicationStrategy.write(position, encryptedBlockToWriteBackToServer)) {
            logger.error(prefix + "Unable to write swap partner to communicationStrategy: \n" + swapPartner.toString());
            return null;
        }
    ...
    private boolean maintenanceJob(Map<Integer, Map<Integer, BlockLookahead>> accessStash,
...
//        Write back swap stash
    ...
    Map<Integer, Map<Integer, BlockLookahead>> getAccessStash() {
        ...
        for (int i = beginIndex; i < endIndex; i++) {
            BlockEncrypted blockRead = communicationStrategy.read(i + offset);
...
//        The index might not be part of the dummy data, so it is added for each block
    ...
    BlockLookahead[] getSwapStash() {
        ...
        for (int i = 0; i < matrixHeight; i++) {
            res[i] = decryptToLookaheadBlock(communicationStrategy.read(i + flattenedArrayOffSet + offset));
        }
        return res;
    }
