...
public class AccessStrategyPathMult implements AccessStrategy {
    ...
    public AccessStrategyPathMult(int size, int bucketSize, byte[] key, Factory factory, AccessStrategy accessStrategy,
...
//                positionsToRead.add(position + i + offset);
                int index = position + i + offset;
                BlockEncrypted encryptedBlock = communicationStrategy.read(index);
                if (encryptedBlock == null) {
                    logger.error(prefixString + "Block fetch with index: " + index + " were null");
                    return false;
                }
...
//        if (encryptedBlocks == null || bucketSize * L != encryptedBlocks.size()) {
        if (bucketSize * L != encryptedBlocks.size()) {
...
//            Encrypts all pairs
            ...
            for (int i = 0; i < blocksToWrite.size(); i++) {
                boolean w = communicationStrategy.write(arrayPosition + i + offset, encryptedBlocksToWriteTmp.get(i));
                if (!w) {
                    logger.error(prefixString + "Unable to write block to server");
                    return false;
                }
            }
        }
